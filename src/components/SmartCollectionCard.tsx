...
export const SmartCollectionCard = ({ id, name, items, isUploaded, onToggleUpload }: SmartCollectionCardProps) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);

  const uploadedCount = items.filter(item => isUploaded(item.id)).length;
  ...
  const sortedItems = useMemo(() => {
    return [...items].sort((a, b) => (b.releaseDate || b.year || '').localeCompare(a.releaseDate || a.year || ''));
  }, [items]);
  ...
  // Get poster from first item
  const poster = items[0]?.poster || '/placeholder.svg';
    ...
    <div className={`rounded-xl border transition-all duration-200 ${
      ...
      >
        ...
        <div className="relative flex-shrink-0">
          ...
          />
          <div className="absolute -top-1 -right-1 bg-primary text-primary-foreground text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center">
            {items.length}
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {items.length} médias • {uploadedCount} uploadé{uploadedCount > 1 ? 's' : ''}
          </p>
          {/* Progress bar */}
          ...
          >
            {allUploaded ? (
              ...
              <span className="flex items-center gap-1.5">
                ...
                {partiallyUploaded ? `${uploadedCount}/${items.length}` : 'Uploader tout'}
              </span>
            )}
          </button>
          {isExpanded ? (
              ...
              >
                ...
                <div className={`absolute inset-0 flex items-center justify-center transition-opacity ${
                  ...
                  {isUploaded(item.id) ? (
                    ...
                    <Clock className="w-6 h-6 text-white opacity-0 group-hover:opacity-100 transition-opacity drop-shadow-lg" />
                  )}
                </div>
